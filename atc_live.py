...
def record_audio_ptt(fs=44100):
    ...
    with sd.InputStream(samplerate=fs, channels=1, callback=callback):
        while keyboard.is_pressed('+'):
            sd.sleep(50) # Small sleep to prevent high CPU usage
...
async def run_atc_loop():
    ...
    while True:
        # 1. RECORD YOUR VOICE (Push-to-Talk)
        record_audio_ptt()
        ...
        # 2. TRANSCRIBE (Voice to Text via Groq Whisper)
        print("Transcribing...")
        ...
        system_prompt = (
            ...
            f"The pilot's tail number is {tail} (If 'UNKNOWN', use their stated callsign). Aircraft state: {alt_ft:.0f}ft MSL, currently {location_status}. "
